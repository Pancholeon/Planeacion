...
index a6a32c12c9ef15176ca10d760722a05c56d76d5c..af51f9fd8b4198b190757d574d635b29de916782 100644
--- a/streamlit_app.py
+++ b/streamlit_app.py
@@ -105,181 +105,251 @@ def infer_geo_columns(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str], O
...
+        costo_total = costo_cluster + (peso_radicacion * costo_radicacion / 100.0) + (peso_dispersion * costo_dispersion) + costo_particion
         ...
         for j, ent in enumerate(entrevistadores):
@@ -309,50 +379,131 @@ def asignacion_balanceada_geografica(
             subset[lon_col].mean(),
         )
...
@@ -364,72 +515,75 @@ def build_summary(
     ...
     if "asignado_a" in df.columns:
         ...
         fig.add_trace(
             go.Scattermap(
                 ...
                 textfont={"size": 13},
                 hoverinfo="skip",
@@ -483,63 +637,81 @@ lat_col = st.sidebar.selectbox(
...
@@ -576,162 +748,200 @@ run = st.button("Ejecutar asignación compacta", type="primary")
 if run:
     try:
         df_work = df_geo.copy()
